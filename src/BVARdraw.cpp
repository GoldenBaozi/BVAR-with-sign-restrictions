...
// [[Rcpp::export]]
double loglik(mat Sigma, vec alpha, vec y, mat X, int m, int T)
{
...
// [[Rcpp::export]]
List gibbs_sampler(mat Y, mat X, List priors, int burn_in = 100, int draw = 1000, int thin = 1, int post_save = 500, int post_method = 0)
{
    // priors
    vec alpha_bar = as<vec>(priors[0]);
    mat Sigma_alpha = as<mat>(priors[1]);
    mat Sigma_alpha_inv = inv_sympd(Sigma_alpha);
    mat Sigma_sigma = as<mat>(priors[2]);
    ...
    // save draws
    mat alpha_draws(K, draw, fill::zeros);
    cube Sigma_draws(m, m, draw, fill::zeros);
    vec loglik_draws(draw, fill::zeros);
    // save posterior draws
    mat alpha_post(K, post_save, fill::zeros);
    cube Sigma_alpha_post(K, K, post_save, fill::zeros);
    cube Sigma_sigma_post(m, m, post_save, fill::zeros);
    vec nu_post(post_save, fill::zeros);
    // save posterior out
    vec alpha_mean_post(K, fill::zeros);
    mat alpha_cov_post(K, K, fill::zeros);
    mat Sigma_mean_post(m, m, fill::zeros);
    int nu_post1 = 0;
